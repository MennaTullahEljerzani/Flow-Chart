...
Input::Input(window* pW) { pWind = pW; } //point to the passed window

//Wait for mouse click
void Input::GetPointClicked(Point& P) const { pWind->WaitMouseClick(P.x, P.y); }


string Input::GetString(Output* pO, string s) const
{
	string Str = s;
	if (Str != ""){ pO->PrintMessage(Str); }
	char Key;
	while (1)
	{
...
double Input::GetValue(Output* pO, double s) const				// Reads a double value from the user 
{
	...
	while (!exit)
	{
		...
		if (Str[0] == '-') 
		{		
			// If the input number is negative (first char = '-')
		...
		{
			// NOTE //
			// ASCII CODE: 48 is'0', 57 is '9'
			if (((int)Str[i] < '0' || (int)Str[i]>'9') && Str[i] != '.')
			{
				pO->PrintMessage("Invalid value ,please enter numbers only ,press any key to retry ");
				char x;
				pWind->WaitKeyPress(x);
...
string Input::GetCompOperator(Output* pO) const
{
	...
	while (1)
	{
		pO->PrintMessage("Enter a comparison operator (==, !=, <, <=, > or >=): ");
		comparison_operator = GetString(pO,comparison_operator);
			...
			comparison_operator == "<=" || comparison_operator == ">" || comparison_operator == ">=")
		{
			...
			pO->PrintMessage("Invalid entered value Please enter comparison operator only(+, -, *or /)only: ");
			GetPointClicked(P);
		}
...
string Input::GetVariable(Output* pO) const
{
	...
	variable = GetString(pO,variable);
...
ActionType Input::GetUserAction(Point& P) const
{
	...
	if (UI.AppMode == DESIGN)	//application is in design mode
	{
		...
		{
			// NOTE //
			// switch cases could also be used instead of if's and else if's
	...
	{		//[1] If user clicks on the Toolbar
		...
		if (y < UI.height && y >= UI.height - UI.StBrWdth){ return STATUS; }
	}
}
